...
PASSWORD=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 64 | head -n 1) &&
    ...
    sed -e "s#username: root#username: redmine#" -e "s#password: \"\"#password: \"${PASSWORD}\"#" -e "w/opt/oliveheart/redmine-${MAJOR}.${MINOR}.${PATCH}/config/database.yml" /opt/oliveheart/redmine-${MAJOR}.${MINOR}.${PATCH}/config/database.yml.example &&
