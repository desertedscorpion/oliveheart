...
RUN chmod 0500 /opt/oliveheart/oliveheart && cd /opt/oliveheart && wget http://www.redmine.org/releases/redmine-${MAJOR}.${MINOR}.${PATCH}.tar.gz && tar xfzv redmine-${MAJOR}.${MINOR}.${PATCH}.tar.gz && rm /opt/oliveheart/redmine-${MAJOR}.${MINOR}.${PATCH}.tar.gz && gem install bundler && echo systemctl enable oliveheart.service
CMD ["/usr/sbin/init"]